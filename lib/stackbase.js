...
//  }
  ...
  BiwaScheme.Set = Class.create({
    ...
    set_intersect : function(s2){
      if(!s2 instanceof Set) throw new Error("set_intersect: arguments must be a set");
      var o = new Set();
      for(var i=0; i<this.arr.length; i++)
    ...
    set_minus : function(s2){
      if(!s2 instanceof Set) throw new Error("set_minus: arguments must be a set");
      var o = new Set();
      for(var i=0; i<this.arr.length; i++)
        if(!s2.member(this.arr[i]))
          o.add(this.arr[i]);
      ...
      if(x instanceof Symbol){
        ...
        case Sym("lambda"):
          ...
          if (vars instanceof Pair){ // (lambda (...) ...)
            ...
            ret = this.find_sets(body, v.set_minus(new Set(vars)));
          }
          break;
        case Sym("if"):

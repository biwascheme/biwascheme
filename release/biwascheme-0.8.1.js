/*
 * BiwaScheme 0.8.1 - R6RS/R7RS Scheme in JavaScript
 *
...
var BiwaScheme = (function () {
  ...
  const VERSION = "0.8.1";
  const GitCommit = "302f0b2ed47aaf192c414d5e64d15864d971ef17";
  ...
  const make_simple_assert = function (type, test, _fname) {
    return make_assert(function (fname, obj, opt) {
      ...
      if (!test(obj)) {
        throw new BiwaError(
          option + type + " required, but got " + to_write(obj)
        );
      }
    });
  };
  ...
  define_syntax("cond", function (x) {
    ...
    if (!(clauses instanceof Pair) || clauses === nil) {
      throw new BiwaError(
        "malformed cond: cond needs list but got " + write_shared(clauses)
      );
    }
    ...
    if (x instanceof Pair) {
      if (x.car === Sym("quasiquote")) {
        ...
        if (depth > 0) {
          ...
          // i.e. x.car === Sym("unquote-splicing")
  ...
  define_syntax("case-lambda", function (x) {
    ...
    clauses.reverse().forEach(function (clause) {
      ...
      if (formals === nil) {
        ...
        throw new BiwaError(
          "case-lambda: invalid formals: " + to_write(formals)
        );
      }
    });
  ...
  define_syntax("delay", function (x) {
    ...
    if (x.cdr.cdr !== nil) {
      throw new BiwaError(
        "malformed delay: too many arguments: " + write_shared(x)
      );
    }
    var expr = x.cdr.car;
  ...
  define_syntax("delay-force", function (x) {
    ...
    if (x.cdr.cdr !== nil) {
      throw new BiwaError(
        "malformed delay-force: too many arguments: " + write_shared(x)
      );
    }
    var expr = x.cdr.car;
    // Expand into call of internal function

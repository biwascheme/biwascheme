/*
 * BiwaScheme 0.7.1 - R6RS/R7RS Scheme in JavaScript
 *
 * Copyright (c) 2007-2020 Yutaka HARA (http://www.biwascheme.org/)
...
(function () {
  ...
  // Current version.
  var VERSION = _$1.VERSION = '1.10.2';
    ...
    each(names, function(name){
      ...
      proto[name] = function(/*arguments*/){
        if(!this.hasOwnProperty("cached_"+name)){
          this["cached_"+name] = this["compute_"+name].apply(this, toArray(arguments));
        }
  ...
  const Hashtable = Class.create({
    ...
    create_copy: function(mutable){
      ...
      each(keys(this.pairs_of), bind(function(hashed){
        var pairs = this.pairs_of[hashed];
  ...
  // string ports (srfi-6)
  //
  Port$1.StringOutput = Class.extend(new Port$1(false, true), {
    initialize: function(){
      this.buffer = [];
    },
  ...
  Port$1.StringInput = Class.extend(new Port$1(true, false), {
    initialize: function(str){
      this.str = str;
    },
  ...
  Port$1.NullInput = Class.extend(new Port$1(true, false), {
    initialize: function(){
    },
    get_string: function(after){
  ...
  Port$1.NullOutput = Class.extend(new Port$1(false, true), {
  ...
  Port$1.CustomOutput = Class.extend(new Port$1(false, true), {
    initialize: function(output_function){
      this.output_function = output_function;
    },
    put_string: function(str){
  ...
  Port$1.CustomInput = Class.extend(new Port$1(true, false), {
  ...
  Port$1.current_input  = new Port$1.NullInput();
  Port$1.current_output = new Port$1.NullOutput();
  Port$1.current_error  = new Port$1.NullOutput();
  ...
  const to_write = function(obj){
    if(obj === undefined)
      ...
      return "#(" + map(obj, function(e) { return to_write(e); }).join(" ") + ")";
  ...
  const find_cyclic = function(obj, known, used){
    ...
    each(items, function(item){
      if(typeof(item)=='number' || typeof(item)=='string' ||
        ...
        item === nil$1 || item instanceof BiwaSymbol) return;
  ...
  const GitCommit = "1bfb393a46219e14a226ed479d60352a2ddbec1b";
  ...
  const Parser = Class.create({
    ...
    getObject0: function() {
      ...
      if( s || t == '(' || t == '#(' || t == '[' || t == '#[' || t == '{' || t == '#{' ) {
        return s ? new Pair(Sym(s), new Pair( this.getObject(), nil$1 ))
  ...
  const Syntax = Class.create({
    ...
    transform: function(x){
      if (!this.func){
        throw new Bug$1("sorry, syntax "+this.sname+
  ...
  const Interpreter = Class.create({
    ...
    initialize: function(){
      ...
      this.call_stack = last_interpreter ? clone(last_interpreter.call_stack) : [];
    ...
    save_stack: function(s){
      ...
      return { stack: v, last_refer: this.last_refer, call_stack: clone(this.call_stack), tco_counter: clone(this.tco_counter) };
    },
        ...
        case "apply": //extended: n_args as second argument
          ...
          if(isClosure(func)){
            ...
            if(result instanceof Pause){
              ...
              var push_args = reduce(result.args, function(opc, arg){
  ...
  const Compiler = Class.create({
    ...
    find_sets: function(x, v){
      ...
      if(ret == null)
        throw new Bug$1("find_sets() exited in unusual way");
      else
        return ret;
    },
    ...
    find_free: function(x, b, f){
      ...
      if(ret == null)
        throw new Bug$1("find_free() exited in unusual way");
      else
        return ret;
    },
  ...
  //        return ret;
    ...
    _compile_define: function(x, next){
      ...
      if(first instanceof BiwaSymbol){    
        if (rest === nil$1) { // 1. (define a)
          x = undef;
        }
        else {
          if (rest.cdr === nil$1) // 2. (define a 1)
            x = rest.car;
          else // 3. (define a 1 2)
            throw new BiwaError("Invalid `define': "+x.to_write());
        }
  ...
  Record.RTD = Class.create({
    ...
    initialize: function(name, parent_rtd, uid, sealed, opaque, fields){
      ...
      if(uid){
        ...
        this.uid = this._generate_new_uid();      this.generative = true;
      }
  ...
  const alias_libfunc = function(fname, aliases) {
    if (CoreEnv[fname]) {
      if (isArray(aliases)) {
        map(aliases, function(a) { alias_libfunc(fname, a); });
  ...
  Enumeration.EnumSet = Class.create({
    ...
    is_subset: function(other){
      // Check elements
      if(some(this.symbols, function(sym){
    ...
    union: function(other){
      var syms = filter(this.enum_type.members, bind(function(sym){
                   return contains(this.symbols, sym) ||
                          contains(other.symbols, sym);
                 }, this));
  ...
  const Values$1 = Class.create({
    ...
    to_write: function(){
      return "#<Values " +
               map(this.content, to_write).join(" ") +
             ">";
    }
  });
  ...
  define_libfunc("map-with-index", 2, null, function(ar){
    ...
    return Call.multi_foreach(lists, {
      call: function(xs){ 
        var args = map(xs, function(x){ return x.car });
        args.unshift(i);
        i++;
  ...
  define_libfunc("with-output-to-port", 2, 2, function(ar){
    ...
    var original_port = Port$1.current_output;
    Port$1.current_output = port;

    return new Call(proc, [port], function(ar){
      port.close();
      Port$1.current_output = original_port;
  ...
  define_libfunc("js-new", 1, null, function(ar, intp){
    ...
    var ctor = ar.shift();
    if (isString(ctor)) ctor = eval(ctor);
  ...
  define_libfunc("sleep", 1, 1, function(ar){
    ...
    assert_real(sec);
    return new Pause(function(pause){
      setTimeout(function(){ pause.resume(nil$1); }, sec * 1000);
    });
  });
    ...
    if(x.cdr === nil$1){
      ...
      each(clauses.to_array().reverse(), function(clause){
        if(clause.car === Sym("else")){
          ...
          ret = List(
            Sym("if"),
            new Pair(Sym("or"), array_to_list(map(clause.car.to_array(), function(d){
                return List(Sym("eqv?"),
                            tmp_sym,
                            List(Sym("quote"), d));
            }))),
  ...
  define_syntax("let*", function(x){
    ...
    if(binds === nil$1)
      return new Pair(Sym("let"), new Pair(nil$1, body));
  ...
  define_syntax("let-values", function(x) {
    ...
    var binding = new Pair(tmpsym,
         new Pair(
            new Pair(Sym("lambda"), new Pair(nil$1,
                     new Pair(init, nil$1))),
            nil$1));
    let_bindings = new Pair(binding, let_bindings);

    var formals = item.car;
    let_star_values_bindings = new Pair(new Pair (formals, new Pair(new Pair(tmpsym, nil$1), nil$1)),
      ...
      ret = new Pair(Sym("let"),
         new Pair(let_bindings,
            new Pair (let_star_values, nil$1)));
      return ret;
  ...
  define_libfunc("length", 1, 1, function(ar){
    assert_list(ar[0]);
    var n = 0;
    for(var o=ar[0]; o!=nil$1; o=o.cdr)
      n++;
    return n;
  });
  ...
  define_libfunc("map", 2, null, function(ar){
    ...
    return Call.multi_foreach(lists, {
      ...
      call: function(xs){
        return new Call(proc, map(xs, function(x){ return x.car }));
      },
  ...
  define_libfunc("string", 0, null, function(ar){
    ...
    for(var i=0; i<ar.length; i++)
      assert_char(ar[i]);
    return map(ar, function(c){ return c.value }).join("");
  });
  ...
  define_libfunc("string->list", 1, 1, function(ar){
    assert_string(ar[0]);
    return array_to_list(map(ar[0].split(""), function(s){ return Char.get(s[0]); }));
  });
  define_libfunc("list->string", 1, 1, function(ar){
    assert_list(ar[0]);
    return map(ar[0].to_array(), function(c){ return c.value; }).join("");
  });
  ...
  define_libfunc("for-all", 2, null, function(ar){
    ...
    return Call.multi_foreach(lists, {
      call: function(pairs){
        return new Call(proc, map(pairs, function(x){ return x.car }));
      },
  ...
  define_libfunc("fold-left", 3, null, function(ar){
    ...
    return Call.multi_foreach(lists, {
      call: function(pairs){
        var args = map(pairs, function(x){ return x.car });
        args.unshift(accum);
        return new Call(proc, args);
      },
  ...
  define_syntax("unless", function(x){
    ...
    return new Pair(Sym("if"),
             new Pair(new Pair(Sym("not"), new Pair(test, nil$1)),
               new Pair(new Pair(Sym("begin"), body),
                 new Pair(undef, nil$1))));
  });
  ...
  define_libfunc("record-type-field-names", 1, 1, function(ar){
    assert_record_td(ar[0]);
    return map(ar[0].fields, function(field){ return field.name; });
  });
  ...
  define_libfunc("call-with-string-output-port", 1, 1, function(ar){
    ...
    var port = new Port$1.StringOutput();
  ...
  define_libfunc("read", 0, 1, function(ar){
    var port = ar[0] || Port$1.current_input;
    assert_port(port);
  ...
  define_libfunc("write-char", 1, 2, function(ar){
    var port = ar[1] || Port$1.current_output;
    assert_char(ar[0]);
    port.put_string(ar[0].value);
    return undef;
  });
  define_libfunc("newline", 0, 1, function(ar){
    var port = ar[0] || Port$1.current_output;
    port.put_string("\n");
    return undef;
  });
  ...
  define_syntax("define-enumeration", function(x){
    ...
    define_syntax(type_name, function(x){
      ...
      assert(contains(enum_type.members, arg),
  ...
  define_libfunc("open-input-string", 1, 1, function(ar){
    assert_string(ar[0]);
    return new Port$1.StringInput(ar[0]);
  });
  define_libfunc("open-output-string", 0, 0, function(ar){
    return new Port$1.StringOutput();
  });
  define_libfunc("get-output-string", 1, 1, function(ar){
    assert_port(ar[0]);
    if(!(ar[0] instanceof Port$1.StringOutput))
  ...
  define_libfunc("format", 1, null, function(ar){
    if (isString(ar[0])) {
      ...
      ar.shift();
      var port = Port$1.current_output,
          format_str = ar.shift();
    }
    else {
  ...
  var BiwaScheme$1 = {
    TopEnv, CoreEnv, nil: nil$1, undef, debug, max_trace_size, suppress_deprecation_warning,
    Version: VERSION$1, VERSION: VERSION$1, GitCommit,
    isNil, isUndef, isBoolean: isBoolean$1, isString: isString$1, isChar, isSymbol, isPort, isPair, isList,
    ...
    Error: BiwaError, Bug: Bug$1, UserError,
    Hashtable,
    Interpreter,
    Complex, Rational, isNumber: isNumber$2, isComplex, isReal, isRational, isInteger,
  ...
  Console.puts = function(str, no_newline) {
    Port$1.current_output.put_string(str + (no_newline ? "" : "\n"));
  };

  Console.p = function (/*ARGS*/){
    Port$1.current_output.put_string(
      "p> "+map(toArray(arguments), inspect).join(" ")
    );
  };

  const current_input = new Port$1.CustomInput(
    function (callback) {
      var form = $("<form/>");
  ...
  const current_output = new Port$1.CustomOutput(
    function (str) {
      var console;
      var text;
      console = $("#bs-console");
  ...
  const Dumper = Class.create({
    ...
    dump_opc: function(obj, level, nested){
      ...
      times(level, bind(function(){ pad1 += this.dump_pad; }, this));
      times((level+1), bind(function(){ pad2 += this.dump_pad; }, this));
    ...
    dump: function(obj){
      ...
      if(obj instanceof Object){
        ...
        // registers
        each(keys(obj), bind(function(key){
          var value = obj[key];
          if(key!="x" && key != "stack"){
      ...
      (bind(function(n){
          $(".header", this.dump_el(this.n_dumps)).click(bind(function(){
          this.dump_move_to(n);
          this.dump_fold();
        }, this));
      }, this))(this.n_dumps);
